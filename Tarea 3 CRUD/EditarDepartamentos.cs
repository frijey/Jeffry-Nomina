...
namespace Tarea_3_CRUD
{
    public partial class EditarDepartamentos : Form
    {
        ...
        private void EditarDepartamentos_Load(object sender, EventArgs e)
        {
            if (!nuevo)
            {
                Buscar();
        ...
        private void btnguardar_Click(object sender, EventArgs e)
        {
            if (tbnombre.Text != "")
            {
                ...
                if (nuevo)
                {
                    oper.ConsultaSinResultado("INSERT INTO departamento (descripcion, estado) VALUES ('" + tbnombre.Text + "','" + estado + "');");
                    nuevo = false;
                }
                else
                {
                    oper.ConsultaSinResultado("UPDATE departamento SET descripcion = '" + tbnombre.Text + "', estado = '" + estado + "' WHERE id = '" + tbid.Text + "';");
                }
                this.Close();
            }
            else { }            
        ...
        public void Buscar()
        {
            ...
            try
            {
                ds = oper.ConsultaConResultadods("SELECT * FROM departamento WHERE id = '" + id + "';");
            ...
            {
                MessageBox.Show("Hubo un problema al cargar el departamento", "Aviso");
            }
